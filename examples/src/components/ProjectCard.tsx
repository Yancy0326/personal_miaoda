...
const ProjectCard: React.FC<ProjectCardProps> = ({
  ...
  const cardProps = {
    // Base styles
    className: cn("w-full mx-auto mb-8 md:mb-16", "bg-[#221F26] rounded-2xl md:rounded-3xl overflow-hidden", "border border-white/10 shadow-xl", "transform transition-all duration-300"),
  ...
  return <motion.div {...cardProps} whileInView={{
    opacity: [0, 1],
    scale: [0.95, 1]
  }} transition={{
    duration: 0.6,
    ...
    once: true,
    margin: "-10%"
  }} onMouseMove={handleMouseMove} onMouseLeave={resetPosition} style={{
    ...cardProps.style,
    rotateX: springX,
    rotateY: springY
  }}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8 p-6 md:p-8">
        ...
        <div className="flex flex-col">
          ...
          <ul className="space-y-2 md:space-y-4 mb-6">
            {project.results.map((result, i) => <motion.li key={i} className="flex items-start gap-3 text-white/80 text-sm md:text-base" initial={{
            opacity: 0,
            x: -10
          }} whileInView={{
            opacity: 1,
        ...
        <div className="h-[200px] sm:h-[250px] md:h-full overflow-hidden rounded-lg relative">
          {project.image ? <motion.img src={project.image} alt={project.title} className="w-full h-full object-cover" initial={{
          scale: 1.1
        }} whileInView={{
          scale: 1
        }} transition={{
